...
def profile():
    ...
    if form.validate_on_submit():
        if User.authenticate(user1.username, form.password.data):
            user1.username = form.username.data
            user1.email = form.email.data
            user1.image_url = form.imal_url.data or User.DEFAULT_IMAGE_URL
            user1.header_image_url = form.header_image_url.data or User.DEFAULT_HEADER_IMAGE_URL
            user1.bio = form.bio.data
...
def homepage():
    ...
    if g.user:
        messages = (Message.query.filter(
            Message.user_id.in_([ user.id for user in g.user.following ] + [ g.user.id ])
        ).order_by(
