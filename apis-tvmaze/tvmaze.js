...
const tvMapAPIUrl = "http://api.tvmaze.com/";
...
$searchForm.on("submit", async function (evt) {
 ...
 // searchForShowThenDisplay().then(response => {
...
async function getEpisodesToShow(event) {
  const showId = $(event.target).closest(".Show").data("show-id");
  const episodes = await getEpisodesOfShow(showId);
  populateEpisodes(episodes);
}

$showsList.on("click", ".Show-getEpisodes", getEpisodesToShow)
